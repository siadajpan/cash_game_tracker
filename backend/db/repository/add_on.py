...
def get_player_game_addons(user: User, game: Game, db: Session) -> List[AddOn]:
    ...
    """
    addons = (
        db.query(AddOn).filter(AddOn.user_id == user.id, AddOn.game_id == game.id).all()
    )
    return addons


def get_player_game_total_approved_add_on_amount(
    user: User, game: Game, db: Session
) -> float:
    """
